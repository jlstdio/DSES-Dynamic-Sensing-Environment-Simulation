	...
	public static class GeneratorEditors
	{
		...
		public static void DrawSelector (MatrixGenerators.Selector200 gen)
		{
			using (Cell.Padded(1,1,0,0)) 
			{
				using (Cell.LineStd) 
				{
					Draw.Field(ref gen.rangeDet, "Set Range");
					if (Cell.current.valChanged)
					{
						if (gen.rangeDet == Selector200.RangeDet.Transition)
							gen.SwitchRangeMinmaxToTransition();
						else
							gen.SwitchRangeTransitionToMinmax();
					}
				}
				using (Cell.LineStd) 
				{
				...
				else //RangeDet.Transition
				{
					...
					using (Cell.LineStd) 
					{
						...
						Cell.current.Expose(gen.id, "tFrom", typeof(float));
						Draw.AddFieldToCellObj(typeof(MatrixGenerators.Selector200), "tFrom");
					}
					using (Cell.LineStd) 
					{
						Draw.Field(ref gen.tTo, "To");
						Cell.current.Expose(gen.id, "tTo", typeof(float));
						Draw.AddFieldToCellObj(typeof(MatrixGenerators.Selector200), "tTo");
					}
					using (Cell.LineStd) 
					{
						Draw.Field(ref gen.tTransition, "Transition");
						Cell.current.Expose(gen.id, "tTransition", typeof(float));
						Draw.AddFieldToCellObj(typeof(MatrixGenerators.Selector200), "tTransition");
					}
				}
			}
		}
